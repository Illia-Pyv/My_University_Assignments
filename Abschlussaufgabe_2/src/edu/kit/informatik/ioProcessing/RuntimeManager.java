...
public class RuntimeManager {
    ...
    /**
     * This is the constructor of the class. It receives the command line arguments
     * and initializes the program with them.
     * 
     * @param initialParameters
     */
    public RuntimeManager(String[] initialParameters) {
        if (initialParameters != null) {
            for (String parameters : initialParameters) {
                try {
                    result = commandManager.getInitialize().execute(parameters, gameManager);
                } catch (InvalidParametersException invalidParameters) {
                    output.print(invalidParameters.getResult());
                    quit = true;
                    return;
                }
                if (result.getType().equals(ResultType.FAILURE)) {
                    output.print(result);
                    quit = true;
    ...
    private void processIO(String userInput) {
        try {
            result = this.getCommand(userInput).execute(parameters, gameManager);
        } catch (InvalidParametersException invalidParameters) {
            output.print(invalidParameters.getResult());
            return;
        ...
        if (result.getMessage() == null) {
            quit = true;
        } else if (result.getMessage().equals(Messages.WON_GAME) || result.getMessage().equals(Messages.LOST_GAME)) {
            output.print(result);
            quit = true;
        } else {
            output.print(result);
